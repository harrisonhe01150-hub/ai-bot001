 ...
 *
 * @param {{url?:string, token?:string, ttlMs?:number, timeoutMs?:number, fetchFn?:Function}} opts
 * @returns {Promise<{source:'remote'|'stale'|'local', version:string}>}
 *   remote = 这次拉到了新的（或缓存还新鲜，直接用）
 *   stale  = 这次拉失败，沿用上一次成功的快照
...
export async function ensureRemoteKnowledge({
  ...
  inflight = (async () => {
    try {
      ...
      cache = { payload, fetchedAt: Date.now(), version: String(payload.version || '') };
      return { source: 'remote', version: cache.version };
    } catch (e) {
      // 失败不抛：有旧快照就继续用旧的，没有就用本地硬编码那份。
      // 注意这里不动 cache.fetchedAt —— 下一次调用还会再试，不会被 TTL 挡住。
