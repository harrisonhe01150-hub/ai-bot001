// lib/guards.js
// ⚠️ 本文件是 meihui-wecom-kf 仓库 src/guards.js 的副本，两端必须保持一致。
//    改动请先改那边，再原样复制过来。
//
...
// 老板通过企业微信补充的知识里如果带了新联系方式，由 knowledge-store.js 动态加进来，
// 否则新号码会被下面的防护当成编造内容抹掉。
let extraPhones = new Set();
let extraEmails = new Set();

export function setExtraContacts({ phones = [], emails = [] } = {}) {
  extraPhones = new Set(phones.map((p) => String(p).replace(/[\s-]/g, '')));
  extraEmails = new Set(emails.map((e) => String(e).toLowerCase()));
}

/** 当前动态白名单快照（/kb 接口要把它同步给网站，两端防护口径才一致）。 */
export function getExtraContacts() {
  return { phones: [...extraPhones], emails: [...extraEmails] };
}
...
export function sanitizeReply(raw) {
  ...
  text = text.replace(RE_PHONE, (m) => {
    ...
    if (ALLOWED_PHONES.has(d) || extraPhones.has(d)) return m;
  ...
  text = text.replace(RE_EMAIL, (m) => {
    if (ALLOWED_EMAILS.has(m.toLowerCase()) || extraEmails.has(m.toLowerCase())) return m;
